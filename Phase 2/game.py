...
class Game:
    ...
    def run(self):
        ...
        while not self.gameOver:
            ...
            #if (round(time.time()-st_time,0))%10!=0 and agentIndex!=0:
                #agentIndex=0

            agentIndex = ( agentIndex + 1 ) % numAgents

    
            if (round(time.time()-st_time,0))<3:
                if agentIndex>1:
                   agentIndex=0
            elif (round(time.time()-st_time,0))<6:
                if agentIndex>2:
                   agentIndex=0
            elif (round(time.time()-st_time,0))<9:
                if agentIndex>3:
                   agentIndex=0
            if (round(time.time()-st_time,0))<12:
                if agentIndex>4:
                   agentIndex=0
            elif (round(time.time()-st_time,0))<15:
                if agentIndex>5:
                   agentIndex=0
            elif (round(time.time()-st_time,0))<18:
                if agentIndex>6:
                   agentIndex=0
            if (round(time.time()-st_time,0))<21:
                if agentIndex>7:
                   agentIndex=0
            elif (round(time.time()-st_time,0))<24:
                if agentIndex>8:
                   agentIndex=0
            elif (round(time.time()-st_time,0))<27:
                if agentIndex>9:
                   agentIndex=0
            if (round(time.time()-st_time,0))<30:
                if agentIndex>10:
                   agentIndex=0
            elif (round(time.time()-st_time,0))<33:
                if agentIndex>11:
                   agentIndex=0
            elif (round(time.time()-st_time,0))<36:
                if agentIndex>12:
                   agentIndex=0
            if (round(time.time()-st_time,0))<39:
                if agentIndex>13:
                   agentIndex=0
            elif (round(time.time()-st_time,0))<42:
                if agentIndex>14:
                   agentIndex=0
            elif (round(time.time()-st_time,0))<45:
                if agentIndex>15:
                   agentIndex=0
            if (round(time.time()-st_time,0))<48:
                if agentIndex>16:
                   agentIndex=0
            elif (round(time.time()-st_time,0))<51:
                if agentIndex>17:
                   agentIndex=0
            elif (round(time.time()-st_time,0))<54:
                if agentIndex>18:
                   agentIndex=0
            if (round(time.time()-st_time,0))<57:
                if agentIndex>19:
                   agentIndex=0
            elif (round(time.time()-st_time,0))<60:
                if agentIndex>20:
                   agentIndex=0
            elif (round(time.time()-st_time,0))<63:
                if agentIndex>21:
                   agentIndex=0
